...
if (strlen($_SESSION['alogin']) == 0) {
	...
	if (isset($_POST['update'])) {
		...
		$sql = "update  tblcarwashbooking set adminRemark=:message,paidAmount=:amount,paymentMode=:ttype,txnNumber=:transactionno,status='Completed' where id=:id";
				...
				<div class="agile-grids">
									...
									if ($query->rowCount() > 0) {
										foreach ($results as $result) {				?>
											...
											<tr>
												...
												<td>
													<?php
													// $packages = getRecord("tblwashservice", "service", $result->packageType);
													echo getRecord("tblwashservice", "service", $result->packageType);
													?>
												</td>
						...
						<div class="modal-dialog">
							...
							<div class="modal-content">
								...
								<div class="modal-body">
									<form method="post">
										<p>
											<select name="txntype" id="txntype" required class="form-control">
												<option value="">Transaction Type</option>
												<?php foreach (getTableCol('paymentType', 'tblpaymenttypes') as $payType) : ?>
													<option value="<?php echo $payType['id']; ?>">
														<?php echo $payType['paymentType']; ?>
													</option>
												<?php endforeach; ?>
											</select>
										</p>
										...
										<div style="display:flex;">
											<div style="border:1px solid #ddd;padding-top:5px;text-align:center;padding-right:2px;">
												Price&nbsp;
											</div>
											<?php $prices = getServPrice($_GET['bookingid'])[0]; ?>
											<input type="text" name="price" id="priceField" name="priceField" class="form-control" value="<?php echo number_format($prices['price'], 2, '.'); ?>" disabled>
										</div>
										</p>
										...
										<p>
											<input type="submit" class="btn btn-custom" name="update" value="Update">
										</p>
									</form>
								</div>
					...
					<script>
						$(document).ready(function() {
							...
							$("#txntype").on('change', () => {
								if (priceField.attr('hidden')) {
									priceField.removeAttr('hidden');
									priceField.html('2000.00');
								} else {
									priceField.attr('hidden', 'hidden');
								}
							});
